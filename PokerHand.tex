\documentclass[a4paper,10pt]{article}
\usepackage{longtable,geometry}
\usepackage[english]{babel}
\usepackage[latin1]{inputenc}
\usepackage{amsmath, amssymb}
\usepackage[T1]{fontenc}
\usepackage[babel]{csquotes}
\usepackage{multicol}
\usepackage{eurosym}
% \MakeAutoQuote{Â«}{Â»}
\pagestyle{headings}
\geometry{dvips,a4paper,margin=1.5in}

\usepackage[usenames]{color}
\usepackage{amssymb,amsmath} 
\usepackage{listings}
\usepackage{verbatim}
\usepackage{alltt}
\usepackage{moreverb}
\usepackage{txfonts}
\addtolength{\hoffset}{-1.5cm}
\addtolength{\voffset}{-1.5cm}
\addtolength{\textwidth}{3cm}
\addtolength{\textheight}{3cm}
\title{Poker Hand}
\author{by ToF}
\input{LittleHaskeller.tex}

\begin{document}
\lstset{classoffset=0,
        language=Haskell,
	basicstyle=\color{black}\sffamily\small,
	columns=fullflexible,
	tabsize=4,
	tab=$\to$,
	literate={->}{$\to$ }2 {>=}{$\geq$}1 {==>}{$\Longrightarrow$}1 {lll}{$\lambda$}1 {__}{\space}1 
        {♣}{\monoclubs}1 {♦}{\monodiamonds}1 {♥}{\monohearts}1 {♠}{\monospades}1 , 
	numberstyle=\tiny,
	float=tbph,
	extendedchars,
	breaklines,
	frame=none,
	showtabs=false,
	showspaces=false,
	showstringspaces=false,
	keywordstyle=\color{blue},
	identifierstyle=\color{black}\textsl,
	morekeywords={subsequences, comparing, Suit, Card, Value, Hand},
        classoffset=1,
        morekeywords={C, HighCard,Pair,TwoPairs,ThreeOfAKind,FullHouse,Straight,Flush,StraightFlush,FourOfAKind}, keywordstyle={\color{darkgreen}},
        classoffset=0,
	stringstyle=\color{darkgreen},
	commentstyle=\color{red},
	captionpos=b,
	aboveskip=\medskipamount,
	belowskip=\medskipamount}

\setlength{\columnsep}{1cm}	

\parindent = 0cm
\maketitle
\troislambdas

\vspace{9cm}
\input{Chapter1}
\input{Chapter2}
\input{Chapter3}
\input{Chapter4}
\input{Chapter5}
\input{Chapter6}
\end{document}


